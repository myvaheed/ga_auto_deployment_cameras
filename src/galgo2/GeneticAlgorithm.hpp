...
class GeneticAlgorithm
{
   ...
   template <typename K>
   using OutputFunc = void (*)(const std::vector<K>&);
...
public: 
   ...
   template <int...N>
   GeneticAlgorithm(Func<T> objective, int popsize, int nbgen, bool output, OutputFunc<T> customOutput, const Parameter<T,N>&...args);
   template <int N>
   GeneticAlgorithm(Func<T> objective, int popsize, int nbgen,
   		bool output, OutputFunc<T> outputFunction, std::vector<Parameter<T, N> > const& params);
...
private:
   ...
   template <int N>
   void init(std::vector<Parameter<T, N> > const& params);
...
template<typename T> template<int ...N>
GeneticAlgorithm<T>::GeneticAlgorithm(Func<T> objective, int popsize, int nbgen,
		bool output, OutputFunc<T> outputFunction, const Parameter<T, N>&...args) {
	this->Objective = objective;
	this->outputFuncton = outputFunction;
	// getting total number of bits per chromosome
	this->nbbit = sum(N...);
	this->nbgen = nbgen;
	// getting number of parameters in the pack
...
template <typename T> template <int N>
inline void
GeneticAlgorithm<T>::init(std::vector<Parameter<T, N> > const& params)
{
	for (int i = 0; i < params.size(); i++) {
		auto par = params[i];
		// getting Ith parameter initial data
...
void GeneticAlgorithm<T>::run()
{
   ...
   for (nogen = 1; nogen <= nbgen; ++nogen) {
      ...
      // outputting results
      if (output) print();
      if (outputFuncton) outputFuncton(pop(0)->getParam());
